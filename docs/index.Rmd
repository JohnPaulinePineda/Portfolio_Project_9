---
title: 'Supervised Learning : Modelling Numeric Responses for Prediction'
...
# **1. Table of Contents**
|
| This project implements different predictive modelling procedures for numeric responses using various helpful packages in <mark style="background-color: #CCECFF">**R**</mark>. Models applied in the analysis to predict numeric responses included the **Linear Regression**, **Penalized Regression (Ridge, Least Absolute Shrinkage and Selection Operator, ElasticNet)**, **Principal Component Regression**, **Partial Least Squares**, **Averaged Neural Network**, **Multivariate Adaptive Regression Splines**, **Support Vector Machine (Radial Basis Function Kernel, Polynomial Kernel)**, **K-Nearest Neighbors**, **Classification and Regression Trees**, **Conditional Inference Trees**, **Random Forest** and **Cubist** algorithms. The resulting predictions derived from the candidate models were evaluated in terms of their model fit using the r-squared and root mean squred error (RMSE) metrics. All results were consolidated in a [<span style="color: #FF0000">**Summary**</span>](#summary) presented at the end of the document.
|
...
###  1.5.4 Penalized Linear Regression - Elastic Net (PLR_E)
|
...
|
| [Principal Components Regression](https://www.semanticscholar.org/paper/The-relations-of-the-newer-multivariate-statistical-Hotelling/f8b7eada549ed4dd9419d5a3202834d55f0f4748) applies dimensionality reduction to address high multicollinearity among predictors in a linear regression. The algorithm calculates summary indices termed as principal components which are linear combinations of the original predictors by considering the magnitude of their variances. The method of least squares is then applied to fit a linear regression model using the first principal components as predictors, with the optimal number determined using cross-validation.
|
| **[A]** The principal components regression model from the  <mark style="background-color: #CCECFF">**pls**</mark> package was implemented through the <mark style="background-color: #CCECFF">**caret**</mark> package. 
|
| **[B]** The model contains 1 hyperparameter:
...
|
| [Averaged Neural Networks](https://www.cambridge.org/core/books/pattern-recognition-and-neural-networks/4E038249C9BAA06C8F4EE6F044D09C5C) implement fitting of the same neural network model using different random number seeds. All the resulting models are used for prediction by averaging the model scores and translating to continuous responses. Neural networks are comprised of node layers, containing an input layer, one or more hidden layers, and an output layer. Each node, or artificial neuron, connects to another and has an associated weight and threshold. If the output of any individual node is above the specified threshold value, that node is activated, sending data to the next layer of the network. Otherwise, no data is passed along to the next layer of the network.
|
...
|
| [Multivariate Adaptive Regression Splines](https://www.jstor.org/stable/2241837), as a form of piecewise linear regression, adapts a solution to local data regions of similar linear response. Each of the local regions is expressed by a different basis function. Only the basis functions and the associated predictor variables that provide a sizeable contribution to the prediction are considered. The algorithm divides the dataset into pieces using points termed as knots which are identified by assessing each point for each predictor as a potential knot and creating a linear regression model using the candidate features. The point that is able to reduce the most error in the model is deemed to be the knot with the process repeated recursively. Regression functions are then fitted to each piece to form a hinge function. The optimal number of knots are determined using cross-validation.
|
...
|
| **[B]** The model contains 2 hyperparameters:
|      **[B.1]** <span style="color: #FF0000">nprune</span> = number of model terms made to vary across a range of values equal to 5 to 20
|      **[B.2]** <span style="color: #FF0000">degree</span> = interaction degree made to vary across a range of values equal to 1 to 5
|
| **[C]** The cross-validated model performance of the final model is summarized as follows:
...
|
| [Support Vector Machine](http://www.cs.cmu.edu/~pakyan/compbio/references/Drucker_NIPS_1996.pdf) plots each observation in an N-dimensional space corresponding to the number of features in the data set and finds a hyperplane that passes through the data while minimizing its distance with all the data points. The algorithm applies kernel transformation by mapping non-linearly separable data using the similarities between the points in a high-dimensional feature space for improved estimation.
|
...
|
| [K-Nearest Neighbors](https://ieeexplore.ieee.org/document/1053964) use proximity to make predictions about the numeric response of an individual data point. The algorithm examines the values of a chosen number of data points surrounding a target data point given a distance-based similarity metric, in order to make a prediction about the estimated value of the data point. The process involves setting a value for the chosen number of neighbors, calculating the distance between the target point across all instances, sorting the calculated distances, obtaining the average of the top entries and returning the prediction for the target point.
|
...
|             **[A.1.1]** Cross-Validation R-Squared = 0.92538
|             **[A.1.2]** Test R-Squared = 0.90717 
|             **[A.1.3]** Cross-Validation RMSE = 0.55645
|             **[A.1.4]** Test RMSE = 0.63809
|      **[A.2]** **SVM_R: Support Vector Machine - Radial Basis Function Kernel** (<mark style="background-color: #CCECFF">**kernlab**</mark> package)
|             **[A.2.1]** Cross-Validation R-Squared = 0.91551
|             **[A.2.2]** Test R-Squared = 0.90989 
|             **[A.2.3]** Cross-Validation RMSE = 0.59505
|             **[A.2.4]** Test RMSE = 0.62742
|      **[A.3]** **SVM_P: Support Vector Machine - Polynomial Kernel** (<mark style="background-color: #CCECFF">**kernlab**</mark> package)
|             **[A.3.1]** Cross-Validation R-Squared = 0.91167
|             **[A.3.2]** Test R-Squared = 0.90623 
|             **[A.3.3]** Cross-Validation RMSE = 0.59505
|             **[A.3.4]** Test RMSE = 0.63778
|      **[A.4]** **RF: Random Forest** (<mark style="background-color: #CCECFF">**randomForest**</mark> package)
|             **[A.4.1]** Cross-Validation R-Squared = 0.90098
|             **[A.4.2]** Test R-Squared = 0.90057 
|             **[A.4.3]** Cross-Validation RMSE = 0.65419
|             **[A.4.4]** Test RMSE = 0.65571
|
| 
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_9/images/Project9_Summary.png)
|
# **3. References**
|
...
| **[Article]** [Linear Regression Explained, Step by Step](https://machinelearningcompass.com/machine_learning_models/linear_regression/) by Boris Giba
| **[Article]** [Linear Regression for Machine Learning](https://machinelearningmastery.com/linear-regression-for-machine-learning/) by Jason Brownlee
...
| **[Article]** [Ridge Regression Explained, Step by Step](https://machinelearningcompass.com/machine_learning_models/ridge_regression/) by Boris Giba
| **[Article]** [How to Develop Ridge Regression Models in Python](https://machinelearningmastery.com/ridge-regression-with-python/) by Jason Brownlee
| **[Article]** [Introduction to Ridge Regression](https://www.statology.org/ridge-regression/) by Statology Team
...
| **[Article]** [An Introduction to Multivariate Adaptive Regression Splines](https://www.statology.org/multivariate-adaptive-regression-splines/) by Statology Team
...
| **[Publication]** [The Relations of the Newer Multivariate Statistical Methods to Factor Analysis](https://www.semanticscholar.org/paper/The-relations-of-the-newer-multivariate-statistical-Hotelling/f8b7eada549ed4dd9419d5a3202834d55f0f4748) by Harold Hotelling (British Journal of Statistical Psychology)
...
| **[Publication]** [Multivariate Adaptive Regression Splines](https://www.jstor.org/stable/2241837) by Jerome Friedman (The Annals of Statistics)
